...
# SDK map — Shutterstock API Explorer (Python)
...
|  |  |
| --- | --- |
| SDK display name | Shutterstock API Explorer |
| Root package | `shutterstock_api_explorer` |
| Distribution name | `shutterstock-api-explorer` |
...
Alternatively, scope it — `with ShutterstockApiExplorerClient(...) as client:` closes the pool on exit.
...
Alternatively, scope it — `async with AsyncShutterstockApiExplorerClient(...) as client:` closes the pool on exit.

`AsyncClient` (`shutterstock_api_explorer/async_client.py`) mirrors `Client` method for method, each endpoint method a coroutine. It takes the same keywords, except that each client accepts only its own transport and — where the **Async Type** column differs — only its own flavor.

`Client` and `AsyncClient` are aliases of `ShutterstockApiExplorerClient` and `AsyncShutterstockApiExplorerClient` — the names tracebacks and `repr()` show; all four import from the root.
...
Every API group is a property on the client (e.g. `client.audio_api`). Every constructor argument is optional and keyword-only. Sources: `shutterstock_api_explorer/client.py`, `shutterstock_api_explorer/async_client.py`:
...
| --- | --- | --- |
| `Environment` | `shutterstock_api_explorer.server` | `Literal` of the Environments table's names |
| `ServerConfigOrDict` | `shutterstock_api_explorer.server` | keys as the Servers & auth tables read |
| `HttpClient` | `shutterstock_api_explorer.core` | protocol — `send(request: HttpRequest) -> HttpResponse` · `close()` |
| `BasicAuthCredentialsOrDict` | `shutterstock_api_explorer.core` | `BasicAuthCredentials` or a dict: `username: str` · `password: str` |
| `AuthorizationCodeCredentialsOrDict` | `shutterstock_api_explorer.core` | `AuthorizationCodeCredentials` or a dict: `client_id: str` · `client_secret: str \| None` · `redirect_uri: str` · `scopes: list[Scope] \| None` · `state: str \| None` · `pkce: PkceMethod \| None = "S256"` · `prompt_for_authorization_code: AuthorizationCodePrompt` |
| `CustomerAccessCodeScope` | `shutterstock_api_explorer.auth` | `Enum` of the declared scopes |
| `RefreshableTokenSource` | `shutterstock_api_explorer.core` | protocol — `fetch(credentials) -> OAuthTokenRefreshable` · `refresh(credentials, refresh_token) -> OAuthTokenRefreshable \| None` |
| `AuthorizationCodeCredentials` | `shutterstock_api_explorer.core` | `client_id: str` · `client_secret: str \| None` · `redirect_uri: str` · `scopes: list[Scope] \| None` · `state: str \| None` · `pkce: PkceMethod \| None = "S256"` · `prompt_for_authorization_code: AuthorizationCodePrompt` |
| `AsyncHttpClient` | `shutterstock_api_explorer.core` | protocol — `async send(request: HttpRequest) -> HttpResponse` · `async aclose()` |
| `AsyncAuthorizationCodeCredentialsOrDict` | `shutterstock_api_explorer.core` | `AsyncAuthorizationCodeCredentials` or a dict: `client_id: str` · `client_secret: str \| None` · `redirect_uri: str` · `scopes: list[Scope] \| None` · `state: str \| None` · `pkce: PkceMethod \| None = "S256"` · `prompt_for_authorization_code: AsyncAuthorizationCodePrompt` |
| `AsyncRefreshableTokenSource` | `shutterstock_api_explorer.core` | protocol — `async fetch(credentials) -> OAuthTokenRefreshable` · `async refresh(credentials, refresh_token) -> OAuthTokenRefreshable \| None` |
| `AsyncAuthorizationCodeCredentials` | `shutterstock_api_explorer.core` | `client_id: str` · `client_secret: str \| None` · `redirect_uri: str` · `scopes: list[Scope] \| None` · `state: str \| None` · `pkce: PkceMethod \| None = "S256"` · `prompt_for_authorization_code: AsyncAuthorizationCodePrompt` |
...
- **Case A — typed error.** The operation documents at least one error status, so `shutterstock_api_explorer/errors/` declares a union alias over the bodies those statuses map to — `RawError` is always its last arm, for any undocumented status — and `.error` is annotated with that alias. Narrow it with `isinstance`. The operation blocks name the alias and the status each arm maps from.
- **Case B — raw error.** The operation documents no error status; `.error` is `RawError` (`shutterstock_api_explorer/core/results.py`): `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse`.

Core runtime types (`shutterstock_api_explorer/core/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
| --- | --- | --- |
| `ApiError` — raised by every parsed call; `.error` is a Case A alias from `shutterstock_api_explorer/errors/` or `RawError` | `error: E` · `status_code: int` · `response: HttpResponse` | `shutterstock_api_explorer/core/exceptions.py` |
| `ApiResult[T, E]` — returned by every raw call; the `Success[T] \| Failure[E]` union | `payload: T` (on `Success`) · `error: E` (on `Failure`) · `response: HttpResponse` (on both) | `shutterstock_api_explorer/core/results.py` |
| `RawError` | `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse` | `shutterstock_api_explorer/core/results.py` |
...
```python
from shutterstock_api_explorer.core import ApiError, RawError
...
| **A trailing `request_options`** — keyword-only and optional, for per-call overrides such as a timeout or extra headers; every signature ends with it | here (`shutterstock_api_explorer/core/request_options.py`) |
...
**Shapes live only in the source.** Every module under `shutterstock_api_explorer/models/` declares one type plus its input companion, and every module under `shutterstock_api_explorer/errors/` one alias plus the mapper that builds it; no two share a name. Take a type's module from the operation's **Type sources** table. When no retrieved chunk names it, the module is the type name in snake_case under the kind's directory below (`AccessTokenDetails` ↔ `access_token_details.py`; an error alias drops its `Body` suffix: `AddImageCollectionItemsErrorBody` ↔ `add_image_collection_items_error.py`). Never grep for a type.

| Group | Count | Directory (module = `<type_name>.py`) |
| --- | --- | --- |
| Models (`SdkBaseModel` pydantic classes) | 155 | `shutterstock_api_explorer/models/` |
| Enums (`Enum` over `str`) — Python member names + wire values | 65 | `shutterstock_api_explorer/models/enums/` |
| Unions (plain) — `TypeAlias` over the arms | 8 | `shutterstock_api_explorer/models/unions/` |
| Error aliases (one per Case A operation) | 100 | `shutterstock_api_explorer/errors/` |
...
| --- | --- |
| Client (root) | `shutterstock_api_explorer` |
| Operation controllers | `shutterstock_api_explorer.apis` |
| Models | `shutterstock_api_explorer.models` |
| Enums | `shutterstock_api_explorer.models.enums` |
| Unions | `shutterstock_api_explorer.models.unions`, `shutterstock_api_explorer.models` |
| Error aliases | `shutterstock_api_explorer.errors` |
| Core runtime (`ApiError`, `ApiResult`, `RawError`, …) | `shutterstock_api_explorer.core` |
...
**Environments.** `environment=` selects the target environment (`shutterstock_api_explorer/server/environment.py`):
...
**2 servers.** Base-URL templates and override points (`shutterstock_api_explorer/server/server_config.py`):
